...
| 15 | `OWNED_BY_ME` | resolution → workspace | "Lead vinculado à sua carteira."; chips de etapa/responsável quando autorizados; workspace de 4 áreas | nenhum adicional | nenhum adicional | Campos de display disponíveis conforme Domain Resolution ViewModel autorizado (Q3) | Workspace diferente; aba faltando/extra | CORE, DOM | WA+MC |
| 16 | `IN_POOL` | resolution | Mensagem de domínio + CTA "Abrir Pool na Yolen" (quando `can_open_pool`); workspace conforme Q2 | nenhum | nenhum | Nenhuma | Só texto em um canal (padrão atual ManyChat) | CORE, DOM | WA+MC |
| 17 | `OWNED_BY_OTHER` | resolution | Mensagem de domínio (outra carteira) + CTA "Abrir vínculo na Yolen" (quando `can_open_cycle`); workspace conforme Q2 | nenhum | nenhum | Nenhuma | Só texto em um canal; exibição de dados não autorizados | CORE, DOM | WA+MC |
| 18 | `CLOSED_CYCLE` | resolution | "Este ciclo comercial já está encerrado." (ou `user_message`) + CTA canônico; sem captura/análise | nenhum | nenhum | Nenhuma | Copy contraditória; análise disparada | CORE, DOM | WA+MC |
