...
| 42 | allowed claims | Commercial Config | company/produto | `company_commercial_product_profiles.allowed_claims`/`forbidden_claims` | CONFIG_SOURCE | Supabase | admin UI | prompts | **`company_id+config_version_id+product_id`** — correção (achado do Codex, 33ª revisão do PR #274, mesma causa da linha #40): `company_id+product_id` colide entre as linhas `draft` e `published` do mesmo produto; a unicidade real é `(config_version_id, product_id)`, e `loadCommercialConfig()` carrega as duas versões simultaneamente antes de separá-las em bundles | n/a | até edição | LOW | LOW | não | Companion | idem | reutilizar |
