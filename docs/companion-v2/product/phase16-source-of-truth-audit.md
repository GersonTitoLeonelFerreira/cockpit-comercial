...
| 38 | relationship history (primeiro contato, duração, timeline H2) | Customer Memory | cycle/conversa carregada (não lead inteiro) | **Correção (achado do Codex, 3ª revisão do PR #274) — não é MISSING/[Unverified].** `app/lib/server/companion-client-context-loader.ts::loadCompanionClientContext()` já lê mensagens + eventos de ação do ciclo (`companion_action_events`, ver linha #39) + status do ciclo para o escopo atual; `computeCompanionClientRelationship()` (`companion-client-relationship.ts`) produz `first_known_interaction_at`, duração, últimas interações e contagem; `buildCompanionClientTimeline()` (`companion-client-timeline.ts`) monta a timeline a partir disso, devolvendo tudo a CLIENTE. | DERIVED (read-model, não tabela própria) | nenhuma (recalculado a cada request a partir do Message Ledger + `companion_action_events`) | n/a (leitura) | `loadCompanionClientContext()` → CLIENTE | company_id+cycle_id | herda evidência do Message Ledger (mensagens reais) + `companion_action_events` (eventos reais) | recalculado por request | LOW | LOW | não | CLIENTE | Customer Memory/telemetria | **reutilizar — já existe; limitação real: cobre só o ciclo/conversa carregado, não agrega histórico entre múltiplos ciclos do mesmo lead** |
