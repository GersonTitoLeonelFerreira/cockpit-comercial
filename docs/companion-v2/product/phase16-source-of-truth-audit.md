...
| 33 | seller strength | Seller Coaching | turno | `CommercialReadingSellerStrength[]` | DERIVED (sem persistência legível) | só audit log (`normalized_output`) | `stateful-communication-executor.ts` | ANÁLISE | company_id+cycle_id+conversation_key+candidate_state_version (mesma correção da linha #2) | `evidence_message_ids`**/`memory_ids`** (no output) — correção (achado do Codex, 27ª revisão do PR #274): `CommercialReadingSellerStrength` (`commercial-reading-contract.ts:491-504`) tem `memory_ids` além de `evidence_message_ids`, ambos validados por `normalizeSellerStrengths()` via `normalizeReferences()`/`requireExactFields()` — omitir `memory_ids` perderia proveniência baseada em memória, não só em mensagem direta | recalculado por turno, sem histórico legível | **HIGH** | — | não | ANÁLISE | Seller Coaching (16.3) | **decidir persistência** |
