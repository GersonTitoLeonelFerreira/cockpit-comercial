...
| 46 | recommended next action | Opportunity | turno | `current_priority` / `CommercialReadingCrmSuggestion`/`AgendaSuggestion` | mistura DERIVED/OPPORTUNITY_STATE | ver #13/#44 | ver #13/#44 | AGORA | idem | **desdobrado por parcela — correção (achado do Codex, 12ª revisão do PR #274): `current_priority` tem `evidence_message_ids` (idem #13) e pode ser preservado do estado anterior; `CommercialReadingCrmSuggestion`/`AgendaSuggestion` NÃO têm evidência (idem #44, `NONE`) e não são preservados — duram só o turno** | **desdobrado: `current_priority` até resolve/supersede ou preservação (idem #13); sugestões CRM/Agenda só por turno, sem persistência em read-model (idem #44)** | MEDIUM-HIGH | — | **sim — 3 fontes concorrentes de "próxima ação"** | AGORA | Decision State | consolidar |
...
| `CommercialReading.method.adherence` quando `status ∈ {not_configured, insufficient_evidence}` | **NONE — correção (achado do Codex, 12ª revisão do PR #274):** `normalizeMethodAdherence()` computa `requiresGrounding = false` para esses dois status (`commercial-reading-contract.ts:2102-2106`) e passa isso para `normalizeReferences()`, que aceita `evidence_message_ids: []`/`memory_ids: []` sem falhar quando `requireGrounding` é `false` (linhas 1117-1128). Classificar toda `adherence` uniformemente como PARTIAL/FULL esconde esses dois status sem embasamento real. | `commercial-reading-contract.ts:2102-2106,1117-1128` |
...
10. Fechar a lacuna de validação de `lead_id` no caminho `origin_cycle_id`
    ...
    revalidar `lead_id`. **Ampliado (achado do Codex, 12ª revisão do
    PR #274):** a mesma implementação também precisa garantir que o
    ciclo escolhido no fallback (sem `origin_cycle_id`) seja
    cronologicamente anterior (`created_at <` o ciclo atual) — hoje
    `loadDurableMemorySeedForMissingState()` só ordena por
    `created_at DESC` sem esse filtro, então corrigir só a validação
