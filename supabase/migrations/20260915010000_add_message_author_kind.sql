...
-- Adiciona `author_kind` ao ledger canônico de mensagens.
--
-- Até aqui `direction` (incoming/outgoing) era a única informação de
-- autoria persistida. Isso é suficiente no WhatsApp (só existem duas
-- partes: cliente e vendedor humano), mas não no ManyChat, onde uma
-- mensagem `outgoing` pode ter sido enviada por um vendedor humano OU por
-- uma automação/flow do bot — e automação NUNCA pode ser contabilizada
-- como ação do vendedor nem como decisão/objeção do cliente
-- (app/extension/yolen-companion/src/manychat-message-semantics.js já
-- prova essa distinção a partir do DOM real do ManyChat).
--
-- `author_kind` guarda um destes valores, sempre:
--   customer      - evidência/voz do cliente
--   human_agent   - ação humana do vendedor
--   automation    - mensagem enviada por automação/flow/bot
--   unknown       - autoria não pôde ser provada (fail-closed a jusante:
--                   nunca conta como acerto do vendedor nem como decisão
--                   do cliente)
--
-- Contrato compatível com versões antigas da extensão, no mesmo espírito
-- de deletion_reason (migration 20260829010000): `author_kind` é OPCIONAL
-- no payload de captura. Uma extensão que ainda não o envia (ou envia um
-- valor fora do enum) nunca quebra o lote — tanto a normalização em
-- TypeScript (app/lib/companion/capture-ingestion.ts) quanto esta RPC
-- caem no fallback conservador derivado de `direction`
-- (incoming -> customer, outgoing -> human_agent), que é exatamente o que
-- sempre foi verdade no adapter WhatsApp.

alter table public.conversation_messages
  add column if not exists author_kind text;

-- Backfill conservador: todo histórico existente é WhatsApp, onde
-- direction determina author_kind sem ambiguidade.
update public.conversation_messages
set author_kind = case
  when direction = 'incoming' then 'customer'
  when direction = 'outgoing' then 'human_agent'
end
...
begin
  if not exists (
    ...
    alter table public.conversation_messages
      add constraint conversation_messages_author_kind_check
      check (
        author_kind in (
          'customer',
          'human_agent',
...
begin
  ...
  loop
    ...
    then
      v_author_kind := v_item ->> 'author_kind';
    else
      -- Fail-safe (não fail-closed): extensão antiga ou valor
      -- ausente/inválido nunca quebra o lote. Cai no fallback mais
      -- conservador possível, derivado da direction já validada — é
      -- exatamente o que sempre foi verdade no adapter WhatsApp.
      v_author_kind := case
        when v_direction = 'incoming' then 'customer'
        when v_direction = 'outgoing' then 'human_agent'
        else 'unknown'
      end;
    end if;
...
is
  'Ingere versoes com controle causal por mensagem e por dispositivo, preservando o ledger append-only. Distingue exclusao confirmada (deletion_reason=explicit_deletion) de desaparecimento do DOM (deletion_reason=dom_disappearance), nunca promovendo o segundo ao primeiro. Persiste author_kind (customer/human_agent/automation/unknown), com fallback conservador derivado de direction quando o payload nao o informa.';
