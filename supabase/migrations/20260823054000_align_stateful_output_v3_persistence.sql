-- Yolen Companion V2 - Fase 12A
-- Alinha a persistência stateful ao output contract V3.
--
-- Eventos históricos V2 permanecem V2.
-- A tabela aceita V2 histórico e V3 novo.
-- A RPC passa a exigir V3 para novas persistências.
...
begin
  ...
  if position(
    ...
    raise exception
      'Constraint atual não reconhece o contrato V2 esperado';
  end if;
end;
$migration$;
...
begin
  ...
  if v_occurrences <> 1 then
    raise exception
      'A RPC possui % ocorrência(s) inesperada(s) do contrato V2',
      v_occurrences;
  end if;
...
is
  'Persiste novas análises stateful V3 preservando eventos históricos V2.';
...
begin
  ...
  if position(
    ...
    raise exception
      'RPC não foi atualizada para o output contract V3';
  end if;
  ...
  if position(
    ...
    raise exception
      'Constraint não aceita novos eventos V3';
  end if;
