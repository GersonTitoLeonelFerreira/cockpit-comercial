...
interface Props {
  events: LeadEvent[]
  leadCycle?: LeadCycleSummary | null
}
...
function buildSummary(events: LeadEvent[], leadCycle?: LeadCycleSummary | null): SummaryItem[] {
  ...
  if (lastFinal) {
    ...
    if (isWon && leadCycle && leadCycle.status === 'ganho') {
      const parts: ReactNode[] = []
      if (leadCycle.won_total != null) {
        parts.push(
          <span key="val">
            <span style={{ opacity: 0.55 }}>Valor: </span>
            <span style={{ color: '#86efac', fontWeight: 700 }}>{fmtCurrency(leadCycle.won_total)}</span>
          </span>
        )
      }
      if (leadCycle.products?.name) {
        parts.push(
          <span key="prod">
      ...
      if (leadCycle.payment_method) {
        parts.push(
          <span key="pay">
            <span style={{ opacity: 0.55 }}>Pagamento: </span>
            <span>{PAYMENT_METHOD_PT[leadCycle.payment_method] ?? leadCycle.payment_method}</span>
          </span>
        )
      }
      if (leadCycle.won_note) {
        parts.push(
          <span key="note">
...
export default function LeadOperationalSummary({ events, leadCycle }: Props) {
  if (!events || events.length === 0) return null

  const items = buildSummary(events, leadCycle)
