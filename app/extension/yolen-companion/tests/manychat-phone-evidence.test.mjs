...
test('FASE 6: telefone exibido pela própria UI da Yolen nunca é evidência (sem auto-validação)', () => {
  const documentRef = documentOf(`
    <aside id="yolen-companion-panel">
      <label><span>WhatsApp</span><span>5547999990001</span></label>
    </aside>
    <section class="userColumnContent_1">
      <span>Conversa via WhatsApp</span>
    </section>
  ...
  assert.deepEqual(phoneEvidence.resolveTrustedPhone(documentRef), { ready: false, reason: 'phone_unavailable' })

  const withPlatformContact = documentOf(`
    <aside id="yolen-companion-panel"><span>WhatsApp</span><span>5547999990001</span></aside>
    <section class="userColumnContent_1">
      <span>Conversa via WhatsApp</span>
      <div class="mainContent_1">554799990002</div>
    </section>
  `)

  const result = phoneEvidence.resolveTrustedPhone(withPlatformContact)
  assert.equal(result.ready, true)
  assert.equal(result.phone, '554799990002')
})
