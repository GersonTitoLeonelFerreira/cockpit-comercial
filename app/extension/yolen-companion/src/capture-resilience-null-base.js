;(function initYolenCompanionNullBaseRebase(
  ...
  // Firefox pode expor a API da extensão no global isolado do content
  // script enquanto `window.YolenCompanionApi` vive no WindowProxy da
  // página (globalThis !== window nesse realm). A checagem antiga
  // (`root.window === root`) assumia os dois realms sempre iguais e podia
  // nunca instalar o hotfix nesse cenário real — localizamos a API em
  // qualquer um dos dois realms antes de decidir.
  const nullBaseRebaseTarget =
  ...
  if (nullBaseRebaseTarget) {
    api.installNullBaseRebaseHotfix(
      nullBaseRebaseTarget,
    )
  }
})(
