;(function initYolenManyChatPhoneEvidence(root) {
  ...
  // FASE 6: a própria UI da Yolen (painel montado no body) nunca é fonte de
  // evidência — um telefone exibido pelo Companion não pode se validar.
  const OWN_UI_SELECTOR = '#yolen-companion-panel, [data-yolen-platform]'
  ...
  function isWithinOwnUi(element) {
    return typeof element.closest === 'function' && Boolean(element.closest(OWN_UI_SELECTOR))
  }
  ...
  function collectCandidatePhones(documentRef) {
    ...
    for (const element of elements) {
      ...
      try {
        if (isExcludedBySubscriberId(element) || isWithinOwnUi(element)) continue
      } catch {
        continue
      }
