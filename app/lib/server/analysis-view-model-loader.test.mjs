...
// ---------------------------------------------------------------------------
// FASE 16.6 — testes de composição (glue) do loader que alimenta o
// ANÁLISE seller-facing view model.
//
// Mesmo desenho de agora-decision-state-loader.test.mjs (FASE 16.5):
// este loader não reimplementa nenhuma lógica de leitura/memória/
// coaching — a cobertura aqui foca em composição (que tabelas são
// consultadas, isolamento cross-company, propagação de erro de
// autenticação/escopo), não em re-testar a normalização de Commercial
// Reading/Cycle Memory/Method Coaching (cada um já exaustivamente
// testado em seu próprio arquivo).
// ---------------------------------------------------------------------------
...
test('sem estado comercial persistido e sem memória de ciclo: view model disponível mas unavailable_reason=no_reading, nunca inventa leitura', async () => {
  const admin = createFakeAdmin(baseFixtures())

  const vm = await loadAnalysisViewModel({
    admin,
    token: buildToken(),
    cycle_id: CYCLE_ID,
...
test('acesso negado (sem vínculo ativo com a empresa) propaga CompanionClientContextError, nunca um AnalysisViewModel', async () => {
...
test('AnalysisViewModelReadError é exportado e carrega code/status_code/retryable', () => {
  const error = new AnalysisViewModelReadError({
    code: 'TEST_CODE',
