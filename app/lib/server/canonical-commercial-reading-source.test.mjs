...
const STATE_RECORD_ID = '70000000-0000-4000-8000-000000000001'
...
function buildEvent(overrides = {}) {
  return {
    ...
    generated_at: '2026-09-09T16:58:00.000Z',
    ...overrides,
  }
}

function createAdmin({ events = [], error = null } = {}) {
  let fromCalls = 0
    ...
    then(onFulfilled, onRejected) {
      ...
      let rows = events.filter((row) =>
        ...
        this.upperBounds.every(
          (filter) =>
            Date.parse(row[filter.column]) <= Date.parse(filter.value),
        ),
      )
  ...
  return {
    admin: {
      from(table) {
        assert.equal(table, 'companion_commercial_state_events')
        fromCalls += 1
        return new Query()
      },
    },
...
test('carrega somente a Commercial Reading da versão exata do estado atual', async () => {
  const { admin } = createAdmin({ events: [buildEvent()] })
...
test('rejeita leitura stale, outro state_record e evento futuro', async () => {
  for (const event of [
    buildEvent({ candidate_state_version: 3 }),
    buildEvent({
      state_record_id: '70000000-0000-4000-8000-000000000099',
    }),
    buildEvent({ generated_at: '2026-09-09T17:00:00.001Z' }),
  ]) {
    const { admin } = createAdmin({ events: [event] })
    assert.equal(await load({ admin }), null)
  }
})

test('falha fechado quando existem dois eventos para a mesma identidade', async () => {
  const { admin } = createAdmin({
    events: [
      buildEvent(),
      buildEvent({ id: '80000000-0000-4000-8000-000000000002' }),
    ],
  })
...
test('não consulta eventos quando state_read está missing', async () => {
  const { admin, getFromCalls } = createAdmin({ events: [buildEvent()] })
...
test('rejeita contratos incompatíveis', async () => {
  const cases = [
    {
    ...
    },
    {
      ...
      communication: {
        contract_version: 'phase-5.2-communication-v5',
        commercial_reading: buildValidReading({
          contract_version: 'commercial-reading-invalid',
        }),
      },
    },
  ]

  for (const normalized_output of cases) {
    const { admin } = createAdmin({
      events: [buildEvent({ normalized_output })],
    })
    assert.equal(await load({ admin }), null)
  }
})

test('revalidação rejeita provenance de mensagem removida', async () => {
...
test('reference_time da validação precisa representar o mesmo instante', async () => {
  const { admin } = createAdmin({ events: [buildEvent()] })
  ...
  assert.equal(
    await load({
      admin,
      validationContext: {
        reference_time: '2026-09-09T16:59:00.000Z',
      },
    }),
    null,
  )
...
test('erro de query mantém o reader best-effort', async () => {
  const originalError = console.error
  console.error = () => {}
  try {
    const { admin } = createAdmin({
