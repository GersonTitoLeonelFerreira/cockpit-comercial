...
async function composeWorkingSummary({
  ...
  const response = await provider({
    ...
    system_prompt: [
      ...
      'Uma mensagem marcada como "[mensagem de áudio deste participante ainda sem transcrição disponível]" representa um áudio real ainda não transcrito: nunca invente, presuma ou infira o que foi dito nele. Quando isso afetar a compreensão do momento, registre apenas que existe um áudio pendente de transcrição.',
    ...
    return NextResponse.json(
      {
        ...
        data: {
          ...
          pending_audio_transcription_count:
            messagesForPrompt.filter(isPendingAudioTranscription).length,
        },
      },
      {
        status: 200,
