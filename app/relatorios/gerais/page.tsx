...
export default async function RelatoriosGeraisPage() {
  ...
  // --- Meta vs Realidade ---
  const { data: metaRaw } = await supabase.rpc('report_meta_vs_real', {
    p_company_id: companyId,
  })
  ...
  const periodSummary: ReportPeriodSummary | null =
    periodSummaryRaw && periodSummaryRaw.success
      ? {
          ...
          by_owner: Array.isArray(periodSummaryRaw.by_owner)
            ? periodSummaryRaw.by_owner.map((row: any) => ({
  ...
  const metaVal = meta?.meta_empresa ?? goalConfig?.goal_value ?? 0

  const faturReal = periodSummary?.revenue_total ?? 0
...
const hasTopSellerFromPeriod = topSellerWins > 0 || topSellerRevenue > 0
  ...
  const taxaConversao = taxaUsadaNoSimulador > 0 ? taxaUsadaNoSimulador : 0.2
  ...
  const ciclosTrabDia = diasUteisPass > 0 ? trabalhadosPeriodo / diasUteisPass : 0

  // Status
  const pacingRatio = metaVal > 0 ? projecao / metaVal : 0
  const statusMeta =
    pacingRatio >= 0.95 ? 'no_ritmo' : pacingRatio >= 0.7 ? 'atencao' : 'acelerar'
  const statusColor =
    statusMeta === 'no_ritmo' ? DS.green : statusMeta === 'atencao' ? DS.yellow : DS.red
  const statusLabelText =
    statusMeta === 'no_ritmo' ? 'No ritmo' : statusMeta === 'atencao' ? 'Atenção' : 'Acelerar'
          ...
          >
            ...
            <div style={{ marginTop: 16 }}>
              ...
              >
                ...
                <span>
                  Projeção: {toBRL(projecao)} ({projecaoPct.toFixed(0)}%)
                </span>
              </div>
            </div>
            ...
            >
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  ...
                  <b style={{ fontSize: 13, color: DS.textPrimary }}>
                    {(taxaPlanejada * 100).toFixed(1)}%
                  </b>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ fontSize: 11, color: DS.textSecondary }}>
                    Real (ganho/trabalhado)
                  </span>
                  <b
                    style={{
                      fontSize: 13,
                      color: taxaReal >= taxaConversao ? DS.green : DS.yellow,
                    }}
                  >
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <b
                    style={{
                      fontSize: 13,
                      color: ciclosTrabDia >= ciclosPorDia ? DS.green : DS.red,
                    }}
                  >
                    {ciclosTrabDia.toFixed(1)}
                  </b>
                </div>
              </div>
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <b
                    style={{
                      fontSize: 13,
                      color: vendasRestantes > 0 ? DS.yellow : DS.green,
                    }}
                  >
                    {vendasRestantes}
                  </b>
            ...
            >
              <div style={{ fontSize: 12, color: DS.textSecondary, lineHeight: 1.7 }}>
                ...
                {diasUteisRest > 0 ? (
                  <>
                    Precisa faturar{' '}
                    <b style={{ color: DS.textPrimary }}>{toBRL(faturNecessarioDia)}/dia</b>{' '}
                    e trabalhar{' '}
  ...
  <>
    ...
    <b style={{ color: DS.green }}>{toBRL(topSellerRevenue)}</b> em faturamento.
            ...
            <div style={{ marginTop: 10, fontSize: 10, color: DS.textMuted }}>
              Dados calculados priorizando a competência ativa. Ticket médio real:{' '}
              {toBRL(ticketReal)}. Pool atual: {totalPoolAgora}.
              {trabalhadosPeriodo < 30
                ? ' ⚠️ Amostra pequena — dados ganham precisão com mais movimentações.'
                : ''}
            </div>
          </div>
        ) : (
          ...
          >
            ...
            <p style={{ margin: 0, fontSize: 12, color: DS.textMuted }}>
              ...
              <a
                ...
                style={{ color: DS.blue, textDecoration: 'none' }}
              >
                Simulador de Meta
              </a>
              .
              ...
              <div style={{ fontSize: 12, color: DS.textSecondary }}>
                ...
                {worstStageByCount ? (
                  <>
                    {' '}
                    | Etapa mais crítica:{' '}
                    <b style={{ textTransform: 'capitalize', color: DS.red }}>
                      {worstStageByCount}
                    </b>
                  </>
                ) : null}
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 12, color: DS.textSecondary }}>
              Conversão final (Negociação → Ganho):{' '}
              <b style={{ color: DS.textPrimary }}>{finalConv.toFixed(2)}%</b>
            </div>
          </div>
          ...
          {convErr ? (
            ...
            <>
              <p style={{ color: DS.textSecondary, marginTop: 8, fontSize: 12, lineHeight: 1.5 }}>
                Baseado em leads únicos que <b>entraram</b> na etapa e depois <b>progrediram</b>{' '}
                para a próxima.
              </p>
