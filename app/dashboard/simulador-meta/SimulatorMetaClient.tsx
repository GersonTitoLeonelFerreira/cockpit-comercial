...
export default function SimulatorMetaClient({
    ...
    async function init() {
      ...
      try {
        ...
        if (!activeCompanyId) {
          ...
          const membership = membershipData as ActiveCompanyMembership | null
        ...
        if (isAdminUser) {
            const { data: sellerMemberships, error: sellerMembershipsError } = await supabase
              .from('company_memberships')
              .select('user_id, role')
              .eq('company_id', activeCompanyId)
              .eq('is_active', true)
            ...
            if (sellerMembershipsError) throw sellerMembershipsError
  
            const sellerMembershipRows = (sellerMemberships ?? []) as SellerMembershipRow[]
            const sellerIds = Array.from(new Set(sellerMembershipRows.map((seller) => seller.user_id)))
            ...
            if (sellerIds.length > 0) {
              ...
              const sellerProfileRows = (sellerProfiles ?? []) as SellerProfileRow[]
  
              const sellersList = sellerProfileRows
                .map((seller) => ({
                  id: seller.id,
  ...
  useEffect(() => {
    if (!periodStart || !companyId) return
